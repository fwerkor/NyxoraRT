...
`Runtime::load_program()` adds a bounded module-lifecycle layer above single-image loading. It canonicalizes the executable root, recursively discovers relative `DT_NEEDED` files while keeping every dependency inside that root, deduplicates canonical paths, and tolerates dependency cycles. For a bare dependency name, lookup first preserves the existing same-directory behavior, then uses the referring module's `DT_RUNPATH` when present or `DT_RPATH` otherwise. Colon-separated search components may expand `$ORIGIN`/`${ORIGIN}` to the referring module directory; guest `/app0` is mapped to the program root, and ordinary relative components are rooted there. Core `libkernel.prx`/`libScePosix.prx` disk dependencies may still be satisfied by the registered HLE implementation when no file exists. All discovered modules are mapped before linking; exports from every module are then registered before any relocation pass, so circular dependency graphs do not depend on discovery order for symbol visibility. Generic ELF `PT_DYNAMIC` metadata is parsed through the same path as SCE dynamic metadata.

Program initialization follows the dependency graph rather than file order. The main executable's preinit array runs once, dependencies initialize before their dependents, each module runs `DT_INIT` before its init array, and completed modules are recorded only after their full initializer sequence returns. Finalization walks that completed sequence backwards, invoking each fini array in reverse element order before `DT_FINI`. A constructor failure therefore preserves already-completed dependencies for a later unwind instead of rerunning or forgetting them. Once an init or fini phase faults partway through a module, that phase becomes non-retryable rather than risking duplicate constructor/destructor side effects. The whole initialize/finalize phase shares one guest TLS/TCB execution context. `PT_SCE_PROCPARAM` is currently retained only as a validated mapped guest address/size pair; its version-dependent interior is not interpreted or synthesized.

This loader deliberately does not pretend to provide a complete dynamic-linker namespace. Search paths are confined to the main program root: traversal, symlink escape, unsupported `$...` tokens, and literal absolute paths outside guest `/app0` are rejected before mapping. Empty path-list components are ignored rather than inheriting the host process working directory. The current `RPATH`/`RUNPATH` support is per referring module and direct dependency; inherited `RPATH` chains, firmware/system search namespaces, environment-driven lookup, runtime `dlopen`/`dlclose`, module unmapping, and TLS-module deregistration remain absent. Mapping/link failure after discovery is also not yet fully transactional because mapped VM/TLS state has no complete rollback primitive.
...
The HLE registry shares the same `SymbolKey` space as guest exports. `KernelServices` is runtime-owned rather than global and is reached through the existing guest-thread manager scope. The current slice covers process time/counter/frequency, current CPU, POSIX/kernel `mmap`/`munmap`/`mprotect`, virtual/protection queries, direct-memory size/availability/allocation/query/release/mapping, configured/available flexible-memory queries and mapping, a deny-by-default read-only `/app0` file core (`open`/`read`/`close`/`lseek`/`stat`/`fstat`) exposed through both POSIX and kernel NIDs, pthread create/join/timed-join/self/detach/exit, stack-size/detach-state thread attributes, typed mutexes and bounded mutex attributes, clock-aware condition variables with absolute/relative timed waits, POSIX and `scePthread` semaphores, and nanosleep/usleep/sleep/yield calls. POSIX memory/file/semaphore/sleep failures use a per-host-thread guest errno exposed through `__Error`; `scePthread*` and `sceKernel*` wrappers preserve the ORBIS `0x8002xxxx` convention.

`GuestAddressSpace::RegionInfo` is the canonical VM metadata source. Region kind, physical/file offset, memory type, commitment state, and auxiliary GPU protection bits survive protection and unmap splits, so `sceKernelVirtualQuery` can populate the verified 72-byte guest layout without a second VMA table. `mprotect` uses 16 KiB guest-page alignment and can split one mapped guest region while preserving unaffected metadata and bytes. `sceKernelQueryMemoryProtection` reports the tracked subrange and guest protection bits.

The native arena is trimmed to a 16 KiB-aligned managed span. An embedding host may reserve a flexible-memory capacity before guest mappings exist; that partition is carved from the top of the managed span and configured/available queries report real runtime accounting rather than a hardware-sized constant. The remaining span is the current direct-memory capacity. Direct allocations support aligned first-fit placement, physical-hole reuse, partial release, checked release, adjacent query coalescing by memory type, and zeroing on reallocation. Direct backing survives guest unmap/remap because the native reservation remains present with no-access host protection.

Direct mapping is deliberately narrower than console aliasing: today a physical offset has one canonical guest VA, `managed_base + physical_offset`. A requested noncanonical alias returns `ENOTSUP`; ordinary guest mappings that occupy that canonical VA also make the corresponding direct offset unavailable. This avoids pretending that the single native arena is a shared physical backing object. Arbitrary direct aliases require a separate shared-backing model and alias-aware host mappings. System-flexible memory, memory pools/batch mapping, GPU PRT/pool state, and shared file coherence remain unregistered or unsupported for the same reason.

The file slice is intentionally read-only. `/app0` has no implicit host root: an embedding host must configure it, while `Runtime::load_elf()` initializes it from the first executable directory when no root was supplied. Canonical-path checks keep `..` and symlink resolution inside that root, and write/create/truncate/append flags are rejected. Seek/stat/fstat are implemented; directory enumeration and any writable mount policy remain future work.
...
On SysV x86-64 hosts HLE bindings can point directly at host implementations. Windows uses the generated bridge described above: it remaps up to seven integer/pointer arguments and executes host C++ on the real OS stack. Signatures wider than seven integer/pointer arguments still require an explicit bridge extension rather than unsafe casts.
...
The command frontend must be deterministic and usable without Vulkan. Raw graphics and compute submissions first pass through the shared PM4 processor; a backend receives only a normalized ordered command stream. The current state model tracks Type0 writes and the CONFIG, CONTEXT, graphics-SH, compute-SH, and UCONFIG register spaces. `NUM_INSTANCES`, `DRAW_INDEX_AUTO`, and `DISPATCH_DIRECT` become typed commands. Before each draw or dispatch is emitted, the processor reconstructs the currently programmed VS/PS/CS base address from the GCN `PGM_LO`/`PGM_HI` pair, snapshots the available `RSRC1`/`RSRC2` words, and attaches that immutable shader binding to the normalized command. Program bases are decoded from 256-byte units and non-address control bits in the high word are excluded. Packet validation is transactional: an unsupported or malformed packet rejects the submission before its register/state changes are committed or delivered to the backend. Compute queues keep independent frontend state, while the graphics queue has its own state.

Predication, indexed/indirect draws, indirect buffers, memory/event/barrier packets, `CLEAR_STATE` defaults, and LS/HS/ES/GS stage-enable semantics are intentionally rejected or left undiscovered until their state and memory side effects are represented. Skipping an unknown packet or guessing an active shader stage would make later shader/pipeline state silently wrong.
