...
This loader deliberately does not pretend to provide a complete dynamic-linker namespace. Dependency search is currently relative to the referring module and confined to the main program root; `RPATH`/`RUNPATH`, firmware/system search paths, runtime `dlopen`/`dlclose`, module unmapping, and TLS-module deregistration remain absent. Mapping/link failure after discovery is also not yet fully transactional because mapped VM/TLS state has no complete rollback primitive.
