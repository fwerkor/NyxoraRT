...
The condition implementation keeps table lookup and record locking in one order for wait/signal/broadcast/destroy, preventing a destroy from removing the guest-visible object between lookup and waiter enqueue. Two-waiter broadcast and end-to-end `pthread_exit` through `GuestThreadManager::join` are covered by tests. Cancellation cleanup and the wider pthread/mutex attribute surface remain intentionally unregistered until their semantics are modeled.
