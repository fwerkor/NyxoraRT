...
- dependency-aware program loading for same-root `DT_NEEDED` files, two-phase export/relocation linking, dependency-ordered `preinit`/`init`/`fini` execution, and validated raw `PT_SCE_PROCPARAM` ranges without guessing version-specific fields;
...
5. Add explicit module unload/TLS deregistration and broader dependency search semantics only after their lifetime rules can be enforced without invalidating active guest threads.
