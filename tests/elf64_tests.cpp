...
std::vector<std::byte> minimal_elf() {
    ...
    put16(16, asteria::loader::kTypeSceDynExec); put16(18, asteria::loader::kMachineX86_64);
    put32(20, 1); put64(24, 0x120); put64(32, 64); put16(52, 64); put16(54, 56); put16(56, 1);
    put32(64, asteria::loader::kProgramLoad); put32(68, 5); put64(72, 120); put64(80, 0x1000);
...
ASTERIA_TEST(parses_sce_x86_64_elf) {
    auto image = asteria::loader::Elf64Image::from_bytes(minimal_elf());
    ASTERIA_CHECK(image.is_sce_dynamic());
    ASTERIA_CHECK(image.entry() == 0x120);
    ASTERIA_CHECK(image.program_headers().size() == 1);
    ASTERIA_CHECK(image.program_headers()[0].memory_size == 8);
}

ASTERIA_TEST(rejects_bad_elf_magic) {
    auto bytes = minimal_elf();
    bytes[0] = std::byte{0};
    bool threw = false;
    try { (void)asteria::loader::Elf64Image::from_bytes(std::move(bytes)); }
